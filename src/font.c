...
void Font_CDR_DrawCol(struct FontData *this, const char *text, fixed_t x, fixed_t y, FontAlign align, u8 r, u8 g, u8 b)
{
    ...
    switch (align)
    {
        case FontAlign_Left:
            break;
        case FontAlign_Center:
            x -= ((gameloop == GameLoop_Stage) ? FIXED_DEC(Font_CDR_GetWidth(this, text) >> 1,1) : Font_CDR_GetWidth(this, text) >> 1);
            break;
        case FontAlign_Right:
            x -= ((gameloop == GameLoop_Stage) ? FIXED_DEC(Font_CDR_GetWidth(this, text),1) : Font_CDR_GetWidth(this, text));
            break;
    }
