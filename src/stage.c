...
void moveNote(Note *note, int *notex, int *notey)
{
    if (*notey > FIXED_DEC(screen.SCREEN_HEIGHT2 - 45,1))
    {
        *notex = stage.noteshakex + note_x[((note->type + 3) % 8)] - FIXED_DEC(16,1);
        note->move = *notex / 1024;
    }
                ...
                if (note->type & NOTE_FLAG_SUSTAIN_END)
                {
                    if (clip < (24 << FIXED_SHIFT))
                    {
                        ...
                        if (note->type & NOTE_FLAG_MINE)
                            moveNote(note, &note_dst.x, &note_dst.y);
                        else
