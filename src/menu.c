...
static struct
{
    ...
    //Menu assets
    Gfx_Tex tex_back, tex_ng, tex_story, tex_title, tex_back0, tex_bros;
    FontData font_bold, font_arial;
    //finn and jake
    int brosAnim;
    IO_Data arc_bros, arc_bros_ptr[4];
} menu;
...
void Menu_Load(MenuPage page)
{
    ...
    Gfx_LoadTex(&menu.tex_back0, Archive_Find(arc_back, "back0.tim"), 0);
    Mem_Free(arc_back);

    FontData_Load(&menu.font_bold, Font_Bold);
    FontData_Load(&menu.font_arial, Font_Arial);
    //load finn and jake
    menu.arc_bros = IO_Read("\\MENU\\BROS.ARC;1");
    menu.arc_bros_ptr[0] = Archive_Find(menu.arc_bros, "bro0.tim");
    menu.arc_bros_ptr[1] = Archive_Find(menu.arc_bros, "bro1.tim");
    menu.arc_bros_ptr[2] = Archive_Find(menu.arc_bros, "bro2.tim");
...
void Bros_Tick(int speed)
{
    menu.brosAnim += speed;
    if (menu.brosAnim >= 24*3)
        menu.brosAnim = 0;

    Gfx_LoadTex(&menu.tex_bros, menu.arc_bros_ptr[menu.brosAnim/24], 0);
}
...
void Menu_Tick(void)
{
    ...
    //Fallthrough
        case MenuPage_Title:
        {
            ...
            logo_dst.x = screen.SCREEN_WIDTH / 2 - logo_dst.w / 2;
            logo_dst.y = screen.SCREEN_HEIGHT / 2 - logo_dst.h / 2;
            ...
            //Draw finn and jake
            const RECT finn_src = {0, 0, 46, 127};
            const RECT finn_dst = {0, screen.SCREEN_HEIGHT/2 - 169/2 + 40, 62, 169};
            const RECT jake_src = {46, 0, 94, 179};
            const RECT jake_dst = {screen.SCREEN_WIDTH - 126, 0, 126, 240};

            Bros_Tick(10);
            Gfx_DrawTex(&menu.tex_bros, &jake_src, &jake_dst);
            Gfx_DrawTex(&menu.tex_bros, &finn_src, &finn_dst);

            //Draw bg
            RECT bg_dst = {screen.SCREEN_WIDTH / 2 - 592 / 2, 0, 592, screen.SCREEN_HEIGHT};
            Gfx_DrawTex(&menu.tex_back0, &((RECT){0, 0, 255, 255}), &bg_dst);
            Gfx_DrawRect(&bg_dst,68, 130, 176);
            break;
        }
        case MenuPage_Main:
        {
